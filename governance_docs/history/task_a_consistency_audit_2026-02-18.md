...
- Python scan for broken markdown links in `governance_docs/*.md` and `README.md`
...
- Canonical folder layout present: `governance_docs/`, `openclaw/`, `investments/`, `scripts/`, `local_secrets/`, and template-only `credentials/`.
...
- optionally add a short note in `governance_docs/index.md` clarifying that old-path mentions in reports are archival only.
