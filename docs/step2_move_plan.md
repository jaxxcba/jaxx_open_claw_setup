...
Prepare a safe, non-destructive move plan from current structure to the approved target structure.
No files are moved by this document. This is an execution map.

## Assumptions

- All historical versions are preserved for now.
- Duplicates are moved first, deduplicated in Step 3.
...
- [ ] Keep all history files during move
- [ ] Do not delete duplicates yet
- [ ] Update `docs/index.md` after moves
- [ ] Commit as a single non-destructive reorganization change
...
- Risk: secrets still tracked.
  - Mitigation: complete Step 4 immediately after Step 2/3 stabilization.
